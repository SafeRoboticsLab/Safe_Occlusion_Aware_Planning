...
from client.gui import Frame, HUD
...
COLOR_YELLOW_0 = pygame.Color(252, 227, 62)
...
class MapImage(object):
    ...
    def __init__(self, carla_world, carla_map, pixels_per_meter, obstacle_list, show_triggers, show_connections, show_spawn_points, use_cache = False):
        ...
        if os.path.isfile(full_path) and use_cache:
            # Load Image
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, obstacle_list, world_to_pixel, world_to_pixel_width):
        ...
        def draw_obstacles(surface, obstacle_list, color=COLOR_ALUMINIUM_0):
            for obstacle in obstacle_list:
                coord = obstacle.T.tolist() # [[x1,y1], [x2, y2]]
                coord_pixel = [world_to_pixel(val) for val in coord] 
                pygame.draw.polygon(surface, color, coord_pixel)
        ...
        if obstacle_list is not None:
            draw_obstacles(map_surface, obstacle_list)
    ...
    def world_to_pixel(self, location, offset=(0, 0)):
        ...
        if isinstance(location, list):
            x = self.scale * self._pixels_per_meter * (location[0] - self._world_offset[0])
            y = self.scale * self._pixels_per_meter * (location[1]- self._world_offset[1])
        else:
...
class  MapViewer(object):
    ...
    def __init__(self, args, name="Map Viewer"):
        self.name = name
        self.args = args
        self.server_fps = 0.0
        ...
        self.ego_surface = None
        self.actors_surface = None

        

        # rendering parameters
        self._input = InputControl()
        self.dims = (self.args.width, self.args.height)
        ...
        self.ego_following = True


    def start(self, client, obstacle_list = None, follow_ego = True, show_triggers = False, show_connections = False, show_spawn_points = False, show_actor_ids = False):
        """Build the map image, stores the needed modules and prepares rendering in ego Mode"""

        
        
        self.ego_following = follow_ego
        if self.ego_following:
            print("Initializing Carla Map Viewer in ego following mode")
        else: 
            print("Initializing Carla Map Viewer in map mode")
        ...
        self.map_image = MapImage(
            carla_world=client.world,
            carla_map=client.map,
        ...
        self.original_surface_size = min(self.dims)
        ...
        # Used for ego Mode, draws the map contained in a circle with white border
        center_offset = (int(self.dims[0] / 2), int(self.dims[1] / 2))
        pygame.draw.circle(self.border_round_surface, COLOR_ALUMINIUM_1, center_offset, int(self.dims[1] / 2))
        pygame.draw.circle(self.border_round_surface, COLOR_WHITE, center_offset, int((self.dims[1] - 8) / 2))
    ...
    def tick(self, client):
        """Retrieves the actors for ego and Map modes"""
        ...
        self.ego_transform = client.get_ego_transform()
    ...
    def _render_traffic_lights(self, surface, list_tl, world_to_pixel):
        ...
        for tl in list_tl:
            ...
            if self.ego_transform is not None:
                ...
                d = self.ego_transform.location.distance(transformed_tv)
                s = Util.length(tl.trigger_volume.extent) + 5
                if (d <= s):
    ...
    def _render_speed_limits(self, surface, list_sl, world_to_pixel, world_to_pixel_width):
        ...
        for sl in list_sl:
            ...
            # Blit
            if self.ego_following:
                # In ego mode, Rotate font surface with respect to ego vehicle front
                angle = -self.ego_transform.rotation.yaw - 90.0
                font_surface = pygame.transform.rotate(font_surface, angle)
    ...
    def _render_shadow(self, shadow_list, world_to_pixel):
        if shadow_list is None:
            return
        self.shadow_surface.fill(COLOR_BLACK)

        self.shadow_surface.set_alpha(100)
        for right, left in shadow_list:
    ...
    def _render_vehicles_ids(self, list_actors, world_to_pixel):
        ...
        self.vehicle_id_surface.fill(COLOR_BLACK)
        if self.show_actor_ids:
            self.vehicle_id_surface.set_alpha(150)
            for actor in list_actors:
                ...
                if actor[0].attributes['role_name'] == 'ego':
                    color = COLOR_CHAMELEON_0
                ...
                self.vehicle_id_surface.blit(rotated_font_surface, rect)
    ...
    def render(self, frame, shadow_list=None):
        ...
        if self.ego_following:
            ...
            center = (frame.display.get_width() / 2, frame.display.get_height() / 2)
            rotation_pivot = rotated_result_surface.get_rect(center=center)
            frame.display.fill((0,0,0))
            frame.display.blit(rotated_result_surface, rotation_pivot)

            frame.display.blit(self.border_round_surface, (0, 0))
        else:
            # Map Mode
            # Translation offset
            ...
            center_offset = (abs(frame.display.get_width() - self.surface_size) / 2 * scale_factor, 0)
            ...
            Util.blits(self.result_surface, surfaces)
            frame.display.fill((0,0,0))
            frame.display.blit(self.result_surface, (translation_offset[0] + center_offset[0],
                                               translation_offset[1]))
...
if __name__ == "__main__":
    ...
    try:
        ...
        map_view.start(client, follow_ego=False, show_triggers=False, show_connections=False, show_spawn_points=False)
