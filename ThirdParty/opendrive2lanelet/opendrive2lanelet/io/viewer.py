...
class MainWindow(QWidget):
    ...
    def update_plot(self):
        ...
        _check_conflict = isinstance(selected_lanelet, ConversionLanelet)
        ...
        for lanelet in self.current_scenario.lanelet_network.lanelets:
            ...
            if selected_lanelet is not None:
                ...
                elif _check_conflict and selected_lanelet.split is not None and lanelet.lanelet_id in selected_lanelet.split:
                    color = "teal"
                    alpha = 0.5
                    zorder = 5
                    label = "{} split from {}".format(
                        lanelet.lanelet_id, selected_lanelet.lanelet_id
                    )
                elif _check_conflict and selected_lanelet.merge is not None and lanelet.lanelet_id in selected_lanelet.merge:
                    color = "pink"
                    alpha = 0.5
                    zorder = 5
                    label = "{} merge into {}".format(
                        lanelet.lanelet_id, selected_lanelet.lanelet_id
                    )
                elif _check_conflict and selected_lanelet.cross is not None and lanelet.lanelet_id in selected_lanelet.cross:
                    color = "maroon"
                    alpha = 0.5
                    zorder = 5
                    label = "{} cross {}".format(
        ...
        self.laneletsList.setColumnCount(3)
        # lanelet_data = [
        ...
        for lanelet in self.current_scenario.lanelet_network.lanelets:
            try:
                lanelet_data.append((lanelet.lanelet_id, lanelet.description, lanelet.poses.max()))
            except AttributeError:
                lanelet_data.append((lanelet.lanelet_id, None, None))
        ...
        for idx, lanelet in enumerate(lanelet_data):
            ...
            try:
                ...
                self.laneletsList.setItem(idx, 2, QTableWidgetItem(f"{lanelet[2]}"))
            except AttributeError:
                self.laneletsList.setItem(idx, 1, QTableWidgetItem("None"))
                self.laneletsList.setItem(idx, 2, QTableWidgetItem("None"))
