...
class ParametricLaneGroup:
    ...
    def to_lanelet(self, precision: float = 0.5) -> ConversionLanelet:
        ...
        for parametric_lane in self.parametric_lanes:
            ...
            local_left_vertices, local_right_vertices, local_poses = parametric_lane.calc_vertices(
                precision=precision
            )
            ...
            try:
                ...
                poses = np.hstack((poses, local_poses[idx:]))
            except IndexError:
        ...
        #print(self.id_, poses)
        lanelet = ConversionLanelet(
            copy.deepcopy(self), left_vertices, center_vertices, right_vertices, self.id_, poses
        )
    ...
    def to_lanelet_with_mirroring(
        ...
        # TODO thi poses need to be elaborated
    ...
    def _calc_border_positions(self, precision: float) -> np.ndarray:
        ...
        for i, parametric_lane in enumerate(self.parametric_lanes):
            num_steps = int(max(2, np.ceil(parametric_lane.length / float(precision))))+1
            if not i:
                idx = 0
            else:
                idx = 1
            poses = np.append(
                ...
                #np.arange(0, parametric_lane.length, precision)[idx::]
                + self._geo_lengths[i],
            )
        return poses
