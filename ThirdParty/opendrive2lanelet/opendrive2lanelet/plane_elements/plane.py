...
class ParametricLane:
    ...
    def to_lanelet_with_mirroring(
        ...
        num_steps = int(max(3, np.ceil(self.length / float(precision))))+1

        poses = np.linspace(0, self.length, num_steps)
        # poses = np.arange(0, self.length, precision)
        # if poses[-1]<self.length:
        #     poses = np.hstack((poses, self.length))
    ...
    def calc_vertices(self, precision: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
        ...
        poses = np.linspace(0, self.length, num_steps)
        s_start = self.border_group.outer_border_offset
        s_end = s_start+self.length
        if self.reverse:
            poses_abs = s_end - poses
        else:
            poses_abs = s_start + poses

        # s_start = self.border_group.outer_border_offset
        # s_end = s_start+self.length
        # if self.reverse:
        #     poses_abs = np.arange(np.floor(s_end/precision)*precision, s_start, -precision)
        #     if 
        #     if poses_abs[-1] > s_start:
        #         poses_abs = np.hstack((poses_abs, s_start))
        #     poses = s_end - poses_abs
        # else:
        #     poses_abs = np.arange(np.ceil(s_start/precision)*precision, s_end, precision)
        #     if poses_abs[-1] < s_end:
        #         poses_abs = np.hstack((poses_abs, s_end))
        #     poses = poses_abs - s_start

        if poses.size == 0:
            poses = np.array([0])
            if self.reverse:
                poses_abs = np.array([s_end])
            else:
                poses_abs = np.array([s_start])
        ...
        for pos in poses:
            ...
            left_vertices.append(inner_pos)
            right_vertices.append(outer_pos)
        return (np.array(left_vertices), np.array(right_vertices), poses_abs)
