...
class ConversionLaneletNetwork(LaneletNetwork):
    ...
    def __init__(self):
        super().__init__()
        self.new_lanelet_ids = {} # dictionary link OpenDrive lane name to lanelet id
    ...
    def find_lanelet_by_opendrive_id(self, opendrive_id: str) -> ConversionLanelet:
        """Find a lanelet for a given OpenDrive ID.
        Disable natural number check of parent class.

        Args:
          opendrive_id: The parametric name of opendrive to find

        Returns:
          The lanelet object if the id exists and None otherwise

        """
        if opendrive_id not in self.new_lanelet_ids:
            return None
        return self._lanelets.get(self.new_lanelet_ids[opendrive_id])
    ...
    def convert_all_lanelet_ids(self):
        ...
        for lanelet in self.lanelets:
            ...
            #print("old id: ",lanelet.description)
            self.remove_lanelet(lanelet.lanelet_id)
            lanelet.lanelet_id = convert_to_new_lanelet_id(
                lanelet.lanelet_id, self.new_lanelet_ids
            )
            lanelet.predecessor = [
                convert_to_new_lanelet_id(x, self.new_lanelet_ids)
            ...
            if lanelet.adj_left is not None:
                lanelet.adj_left = convert_to_new_lanelet_id(
                    lanelet.adj_left, self.new_lanelet_ids
                )
            if lanelet.adj_right is not None:
                lanelet.adj_right = convert_to_new_lanelet_id(
                    lanelet.adj_right, self.new_lanelet_ids
                )

            if isinstance(lanelet, ConversionLanelet):
                if lanelet.split is not None:
                    lanelet.split = [
                        convert_to_new_lanelet_id(x, self.new_lanelet_ids)
                        for x in lanelet.split
                    ]
                if lanelet.merge is not None:
                    lanelet.merge = [
                        convert_to_new_lanelet_id(x, self.new_lanelet_ids)
                        for x in lanelet.merge
                    ]
                    
                if lanelet.cross is not None:
                    lanelet.cross = [
                        convert_to_new_lanelet_id(x, self.new_lanelet_ids)
                        for x in lanelet.cross
                    ]
    ...
    def join_and_split_possible_lanes(self):
        """Move lanelet boundaries for lanelet split or joins.

        This method provides the functionality to modify the lane boundaries if
        a lane merge into another lane or split from another lane.
        """
    ...
    def find_cross(self, junction_list):
        """
            Iterating through all junctions to check if any of two paths are cross with other

            We assume cross only occurs at junction
        """
        for junction in junction_list:
            for i, path_i in enumerate(junction[:-1]):
                lanelet_i = self.find_lanelet_by_id(path_i)
                if lanelet_i is None:
                    continue # igore road that are not in the filtered out
                # lanelet.center_vertices are 2xN ndarray
                center_line_i = LineString(np.column_stack((lanelet_i.center_vertices[:,0], lanelet_i.center_vertices[:,1])))
                for path_j in junction[i+1:]:
                    lanelet_j = self.find_lanelet_by_id(path_j)
                    # ignore road that are not in the filtered out
                    if lanelet_j is None:
                        continue 
                    
                    # ignore the path that considered as split and merge
                    if lanelet_i.merge is not None and lanelet_j.lanelet_id in lanelet_i.merge:
                        continue

                    if lanelet_i.split is not None and path_j in lanelet_i.split:
                        continue

                    center_line_j = LineString(np.column_stack((lanelet_j.center_vertices[:,0], lanelet_j.center_vertices[:,1])))
                    if center_line_i.intersects(center_line_j):
                        # print(lanelet_i.center_vertices)
                        # print(lanelet_j.center_vertices)
                        # add lane_id to each other's conflict lists
                        if lanelet_i.cross is None:
                            lanelet_i.cross = []
                        if lanelet_j.lanelet_id not in lanelet_i.cross:
                            lanelet_i.cross.append(lanelet_j.lanelet_id)

                        if lanelet_j.cross is None:
                            lanelet_j.cross = []
                        if lanelet_i.lanelet_id not in lanelet_j.cross:
                            lanelet_j.cross.append(lanelet_i.lanelet_id)
...
class _JoinSplitTarget:
    ...
    @property
    def split(self):
        """Lanelet split at start.

        Returns:
          True if lanelet split from other lanelet at start.
        """
        return self._mode >= 1
    ...
    def split_and_join(self) -> bool:
        """Lanelet split at start and joins at end.
    ...
    def _move_borders_if_split_or_join(self):
        ...
        for js_pair in js_pairs:
            ...
            js_pair.move_border(width=distance, linking_side=self.linking_side, precision = self.precision)
...
class _JoinSplitPair:
    ...
    def move_border(
        self, width: Tuple[float, float], linking_side: str, precision: float = 0.5
    ) -> ConversionLanelet:
