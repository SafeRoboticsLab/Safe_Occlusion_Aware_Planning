...
def parse_opendrive(opendrive_input) -> OpenDrive:
    ...
    # Only accept lxml element or string 
    if etree.iselement(opendrive_input):
        root_node = opendrive_input
    elif isinstance(opendrive_input, str):
         # convert string to bytes due to handle the error
         # "Unicode strings with encoding declaration are not supported. Please use bytes input or XML fragments without declaration."
        root_node = etree.fromstring(bytes(opendrive_input, encoding='utf-8'))
    else:
        raise TypeError("Argument root_node is not a xml element or string")
