...
class Agent():
    ...
    """
    def __init__(self, client: SynchronousClient, path, s_0, v, filter = 'vehicle.carlamotors.carlacola', constant_velocity = False):
        self.client = client
        self.path = path
        ...
        #     target_velocity = np.dot(R, np.array([self.v, 0, 0]))
        #     v = carla.Vector3D(target_velocity[0],target_velocity[1],target_velocity[2])
        #     self.actor.set_target_velocity(v)

    def tick(self):
        if self.constant_velocity:
            self.actor.set_target_velocity(carla.Vector3D(0,self.v,0))
        else:
            control = self.agent.run_step(debug=False, unreasonable=True)
            if 'carlacola' not in self.filter:
                control.throttle = min(control.throttle, 0.5)
            control.manual_gear_shift = False
            self.actor.apply_control(control)
            #print(self.actor.get_acceleration())
